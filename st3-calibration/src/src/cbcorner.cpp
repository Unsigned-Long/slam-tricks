...
namespace ns_st3 {
  void CBCorners::adjust() {
    ...
    cv::Point2d op = board[0][0];
    cv::Point2d xp = board[0][cols - 1];
    cv::Point2d yp = board[rows - 1][0];
  ...
  CBCorners CBCorners::read(const std::string &filename) {
    ...
    CBCorners cb(rows, std::vector<cv::Point2d>(cols));
    ...
    while (std::getline(file, str)) {
      ...
      double x = std::stof(str.substr(p2 + 1, p3 - p2 - 1));
      double y = std::stof(str.substr(p3 + 1));
      cb[r][c] = cv::Point2d(x, y);
    }
    file.close();
    return cb;
  }
